...
# Initialization
cars = 20
Values = np.zeros(shape=(cars+1, cars+1))
Pi = np.zeros(shape=(cars+1,cars+1))
Actions = np.arange(-5,6,1)
States = [[i, j] for i in range(cars+1) for j in range(cars+1)]
gamma = 0.9
# Initializing Return and Request probabilities for Loc 1 and Loc 2
poission_len = 10 # Considering only relevant probabilities Since it increases time-complexiity exponentially
e = 2.718282
theta = 2
                ...
                for loc1_ret in range(poission_len):
                    for loc2_req in range(poission_len):
                        for loc2_ret in range(poission_len):
                            ...
                            Values[int(state[0]),int(state[1])] = Values[state[0],state[1]] + p_s_a_ns * ((loc1_req_adj+loc2_req_adj)*10+ 4*add + (-2)*(abs(pi_s)) + gamma*Values[int(next_state[0]),int(next_state[1])])
                ...
                for loc1_ret in range(poission_len):
                    for loc2_req in range(poission_len):
                        for loc2_ret in range(poission_len):
                            ...
                            # Adding One Free Shuttle non-linearity
                            adj_action = action
                            if action > 0:
                                adj_action = action - 1
                            ...
                            action_value = action_value + p_s_a_ns * ((loc1_req_adj+loc2_req_adj)*10 + 4*add + (-2)*(abs(adj_action)) + gamma*Values[int(next_state[0]), int(next_state[1])])
